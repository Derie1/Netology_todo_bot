...
def start(message):
    bot.send_message(message.chat.id, f"Приветствую! Я помогу записать и сохранить список задач. Для работы со мной ознакомьтесь со списком доступных команд:\n{HELP}")
...
def save_to_file(message):
    ...
    bot.send_message(message.chat.id, "Список Ваших задач сохранен в памяти сервера.")

@bot.message_handler(commands=["load"])
def load_from_file(message):
    try:
        tasks.update(from_file(str(message.chat.id)))
        bot.send_message(message.chat.id, "Список Ваших задач восстановлен из памяти.")
    except FileNotFoundError:
        bot.send_message(message.chat.id, "Список Ваших задач в памяти не обнаружен. Создайте задачи и сохраните их при помощи команды '/save'")
...
def add(message):
    ...
    if date == "сегодня":
        today = str(datetime.date.today()).split("-")
        date = f"{today[0]}.{today[1]}.{today[2]}" # date = сегодняшняя дата в формате ГГГГ.ММ.ДД
    elif date == "завтра":
        tomorrow = str(datetime.date.today() + datetime.timedelta(days=1)).split("-")
        date = f"{tomorrow[0]}.{tomorrow[1]}.{tomorrow[2]}" # date = завтрашняя дата в формате ГГГГ.ММ.ДД
